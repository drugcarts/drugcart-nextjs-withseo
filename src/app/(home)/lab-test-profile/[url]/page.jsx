...
const BlogDetail = () => {
  const { labInfoUrl } = useSelector((state) => state.labInfoData);
  const dispatch = useDispatch();
  const params = useParams()

  useEffect(() => {
    dispatch(GetLabInfoUrlService(params?.url));
  }, [params?.url]);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex">
          <div className="w-full flex-none">
            ...
            <div className="space-y-2 p-3" dangerouslySetInnerHTML={{ __html: labInfoUrl.description }} />
          </div>
