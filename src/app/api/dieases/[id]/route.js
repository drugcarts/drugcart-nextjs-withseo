...
export async function PUT(request, { params }) {
    try {
        ...
        const existingDiseases = await Diseases.findById(id);
        if (!existingDiseases) {
            return NextResponse.json({ error: 'Diseases not found' }, { status: 404 });
        }
        ...
        if (body.picture && typeof body.picture === 'object' && body.picture.name) {
            ...
            const newImageKey = `press/${imageFileName(uniqueSuffix)}`;
        ...
        const updatedData = {
            ...
            picture: uniqueSuffix
                ...
                : existingDiseases.picture,
        };

        const updatedDiseases = await Diseases.findByIdAndUpdate(id, updatedData, {
            new: true,
        });
